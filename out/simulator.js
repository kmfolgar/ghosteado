...
exports.inferColType = inferColType;
exports.collectDatasetSchema = collectDatasetSchema;
exports.formatSchemaMarkdown = formatSchemaMarkdown;
exports.generateSimulationPrompt = generateSimulationPrompt;
const fs = require("fs");
...
function inferColType(header) {
    ...
    if (/^id$|_id$|^uid|^uuid|patientid|caseid|recordid|subjectid/.test(h))
        return "id";
    if (/\bage\b|edad|age_at/.test(h))
        return "age";
    if (/date|fecha|dob|birthdt|diagdt|inciddt/.test(h))
        return "date";
    if (/^year$|^yr$|ano|year_/.test(h))
        return "year";
    if (/^sex$|^gender$|sexo|genero/.test(h))
        return "sex";
    if (/icd|morphology|histology|topography|diagnosis/.test(h))
        return "icd";
    if (/count|cases|numero|^n$|^num$|^obs$/.test(h))
        return "count";
    if (/rate|incidence|prevalence|proportion|pct|percent|ratio/.test(h))
        return "rate";
    if (/flag|^is_|^has_|active|bool/.test(h))
        return "boolean";
    if (/name|nombre|apellido|surname|firstname|lastname/.test(h))
        return "name";
    if (/country|region|district|province|state|city|site|pais/.test(h))
        return "category";
    if (/weight|bmi|height|dose|conc|level|score|index/.test(h))
        return "float";
    return "integer";
}
function collectDatasetSchema(datasetRoot, workspaceFolderRel) {
    const walked = walkDatasetFiles(datasetRoot);
    const summaries = walked.files.map((filePath) => summarizeFile(datasetRoot, filePath));
    return {
...
function formatSchemaMarkdown(schema) {
    const lines = [
    ...
    if (schema.truncated) {
        lines.push("", `Note: only the first ${MAX_SCHEMA_FILES} files were scanned for this summary.`);
    }
...
function generateSimulationPrompt(schema, options) {
    const language = options.language === "none" ? "both" : options.language;
    const today = new Date().toISOString().split("T")[0];
    const languageLabel = language === "both" ? "R and Python" : language === "r" ? "R" : "Python";
    const headerLines = [
...
function formatFilePromptSection(file) {
    const lines = [
        `## ${file.relativePath}`,
        `- Format: ${file.format}`,
...
function walkDatasetFiles(datasetRoot) {
    const results = [];
...
function summarizeFile(datasetRoot, filePath) {
    let sizeBytes = 0;
    try {
