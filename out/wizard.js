...
async function runSetupWizard(workspaceRoot, preSelected) {
    ...
    const promptLanguage = languagePick.value;
    const summaryLines = [
    ...
    const confirm = await vscode.window.showWarningMessage(`Ghosteado will do the following:\n\n${summaryLines.filter(Boolean).join("\n")}`, { modal: true }, "Protect Dataset");
    if (confirm !== "Protect Dataset")
        return undefined;
    return {
        sourceFolderPath,
        realDatasetPath,
