...
function activate(context) {
    ...
    (0, logger_1.ensureLogDir)(workspaceRoot);
    guard = new provider_1.GhostGuard(workspaceRoot);
    loadProtectedFolders();
    context.subscriptions.push({ dispose: () => guard.dispose() });
    context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider(provider_1.GHOSTEADO_SCHEME, new provider_1.GhostContentProvider()));
    ...
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(async (doc) => {
        ...
        if (!guard.isGhosted(filePath) || guard.isInBypass(filePath))
            return;
        ...
        const placeholderUri = (0, provider_1.warningUri)(filePath);
        ...
        const mode = cfg.get("notifyOnBlock") ?? "both";
        if (mode === "notification" || mode === "both") {
            const choice = await vscode.window.showWarningMessage(`👻 Ghosteado warning: ${path.basename(filePath)} was opened on the host.`, "Open Anyway", "Resume in Container");
    ...
    context.subscriptions.push(guard.onBlock(() => {
        blockCount += 1;
        updateStatusBar(workspaceRoot);
    }));
    context.subscriptions.push(vscode.commands.registerCommand("ghosteado.protectFolder", async (uri) => {
        const result = await (0, wizard_1.runSetupWizard)(workspaceRoot, uri);
        if (!result)
            return;
        await executeProtectionWizard(workspaceRoot, result);
    }), vscode.commands.registerCommand("ghosteado.setupWizard", async () => {
        const result = await (0, wizard_1.runSetupWizard)(workspaceRoot);
        if (!result)
            return;
        await executeProtectionWizard(workspaceRoot, result);
    }), vscode.commands.registerCommand("ghosteado.unprotectFolder", async (uri) => {
    ...
    }), vscode.commands.registerCommand("ghosteado.regenerateSimulated", async () => {
    ...
    }), vscode.commands.registerCommand("ghosteado.resumeProtectedWorkspace", async () => {
        ...
        (0, logger_1.clearLog)(workspaceRoot);
        blockCount = 0;
        updateStatusBar(workspaceRoot);
...
function deactivate() { }
function registerNoWorkspaceCommands(context) {
    const noWorkspace = () => vscode.window.showWarningMessage("Ghosteado: Open a workspace folder first.");
    context.subscriptions.push(vscode.commands.registerCommand("ghosteado.protectFolder", noWorkspace), vscode.commands.registerCommand("ghosteado.setupWizard", noWorkspace), vscode.commands.registerCommand("ghosteado.unprotectFolder", noWorkspace), vscode.commands.registerCommand("ghosteado.viewSchema", noWorkspace), vscode.commands.registerCommand("ghosteado.copySimulationPrompt", noWorkspace), vscode.commands.registerCommand("ghosteado.regenerateSimulated", noWorkspace), vscode.commands.registerCommand("ghosteado.addContainerProtection", noWorkspace), vscode.commands.registerCommand("ghosteado.resumeProtectedWorkspace", noWorkspace), vscode.commands.registerCommand("ghosteado.showStatus", noWorkspace), vscode.commands.registerCommand("ghosteado.clearLog", noWorkspace));
}
async function executeProtectionWizard(workspaceRoot, result) {
    ...
    if (workspaceFolderRel === "" ||
        workspaceFolderRel.startsWith("..") ||
        path.isAbsolute(workspaceFolderRel)) {
        void vscode.window.showErrorMessage("Ghosteado: Select a dataset folder inside the current workspace, not the workspace root.");
        return;
    }
    ...
    if (path.resolve(result.realDatasetPath).startsWith(path.resolve(workspaceRoot) + path.sep)) {
        void vscode.window.showErrorMessage("Ghosteado: The protected dataset must live outside the workspace.");
        return;
    }
    ...
    if (manifest.protectedDatasets.some((dataset) => dataset.workspaceFolderRel === workspaceFolderRel)) {
        void vscode.window.showInformationMessage(`Ghosteado: "${workspaceFolderRel}" is already protected.`);
        return;
    }
    if (hasOverlappingProtectedDataset(workspaceRoot, sourceFolderPath, manifest.protectedDatasets)) {
        void vscode.window.showErrorMessage("Ghosteado: Nested protected datasets are not supported. Select a top-level dataset folder that does not overlap an existing protected dataset.");
        return;
    }
    let dataset;
    ...
    try {
        const protectionResult = await vscode.window.withProgress({
            ...
            const schema = (0, simulator_1.collectDatasetSchema)(sourceFolderPath, workspaceFolderRel);
        ...
        void vscode.window.showErrorMessage(`👻 Ghosteado: Failed to protect the dataset. ${String(error)}`);
        return;
    }
    if (!dataset) {
    ...
    const infoChoices = ["View Schema", "Copy Prompt"];
    const finalChoices = containerReady
        ? [...infoChoices, "Reopen in Container"]
        : infoChoices;
    const picked = await vscode.window.showInformationMessage(syntheticReady
        ...
        : `👻 Ghosteado protected "${dataset.name}". If no synthetic data exists, the container can still generate code from schema, but it cannot run end-to-end data reads. That is fine if the goal is code generation first. If you want runnable code in-container, synthetic data needs to exist and be mounted there.`, ...finalChoices);
    if (picked === "View Schema") {
        ...
        await (0, devcontainer_1.reopenInContainer)();
    }
    updateStatusBar(workspaceRoot);
}
async function unprotectDataset(workspaceRoot, uri) {
    const manifest = loadManifest(workspaceRoot);
    const dataset = await pickProtectedDataset(workspaceRoot, manifest, uri, "Remove protection from which dataset?");
    if (!dataset)
        return;
    const workspaceFolderPath = path.join(workspaceRoot, dataset.workspaceFolderRel);
    const realDatasetPath = (await loadRealDatasetPath(workspaceRoot, dataset.id)) ??
        resolveLinkedDatasetPath(workspaceFolderPath);
    if (!realDatasetPath) {
        void vscode.window.showErrorMessage(`Ghosteado: Missing host path metadata for "${dataset.name}".`);
        return;
    }
    const confirm = await vscode.window.showWarningMessage(`Remove Ghosteado protection from "${dataset.name}" and move the real dataset back into the workspace?`, { modal: true }, "Remove Protection");
    if (confirm !== "Remove Protection")
        return;
    try {
        ...
        void vscode.window.showErrorMessage(`Ghosteado: Failed to restore the real dataset. ${String(error)}`);
        return;
    }
    manifest.protectedDatasets = manifest.protectedDatasets.filter((item) => item.id !== dataset.id);
    saveManifest(workspaceRoot, manifest);
    ...
    if (manifest.protectedDatasets.length > 0) {
        ...
        (0, devcontainer_1.clearContainerFiles)(workspaceRoot);
    }
    loadProtectedFolders();
    updateStatusBar(workspaceRoot);
    void vscode.window.showInformationMessage(`👻 Ghosteado: Protection removed from "${dataset.name}".`);
}
async function viewSchema(workspaceRoot) {
    const manifest = loadManifest(workspaceRoot);
    const dataset = await pickProtectedDataset(workspaceRoot, manifest, undefined, "View schema for which dataset?");
    if (!dataset)
        return;
    await openSchemaDocument(workspaceRoot, dataset);
}
async function openSchemaDocument(workspaceRoot, dataset) {
    const schema = loadSchema(workspaceRoot, dataset);
    if (!schema) {
        void vscode.window.showErrorMessage(`Ghosteado: No schema summary is available for "${dataset.name}".`);
        return;
    }
    const doc = await vscode.workspace.openTextDocument({
        language: "markdown",
        content: (0, simulator_1.formatSchemaMarkdown)(schema),
    });
    await vscode.window.showTextDocument(doc, { preview: false });
}
async function copySimulationPrompt(workspaceRoot, writePromptFile, forcedDataset) {
    const manifest = loadManifest(workspaceRoot);
    const dataset = forcedDataset ??
        (await pickProtectedDataset(workspaceRoot, manifest, undefined, writePromptFile
    ...
    if (rows === undefined)
        return;
    const prompt = (0, simulator_1.generateSimulationPrompt)(schema, {
    ...
    if (writePromptFile) {
        ...
        await vscode.window.showTextDocument(doc, { preview: false });
        if ((0, devcontainer_1.isInsideContainer)()) {
            void vscode.window.showWarningMessage("👻 Ghosteado: Synthetic workspace prepared. Rebuild or reopen the protected workspace from the host if you need the container mounts updated.");
        }
        else {
            const choice = await vscode.window.showInformationMessage("👻 Ghosteado: Synthetic workspace prepared. Refresh container protection now to update the synthetic overlay?", "Refresh Container Protection", "Later");
    ...
    void vscode.window.showInformationMessage(writePromptFile
...
async function refreshContainerProtection(workspaceRoot, offerReopen) {
    const manifest = loadManifest(workspaceRoot);
    if (manifest.protectedDatasets.length === 0) {
        if (offerReopen) {
            void vscode.window.showWarningMessage("Ghosteado: Protect a dataset first before configuring the container.");
        }
        return false;
    }
    if (!(await (0, devcontainer_1.checkPrerequisites)()))
        return false;
    const language = mergePromptLanguages(manifest.protectedDatasets.map((dataset) => dataset.promptLanguage));
    const containerWorkspaceRoot = (0, devcontainer_1.resolveContainerWorkspaceRoot)(workspaceRoot);
    const mounts = (0, devcontainer_1.buildMountsFromProtectedDatasets)(workspaceRoot, manifest.protectedDatasets.map((dataset) => ({
        workspaceFolderRel: dataset.workspaceFolderRel,
        syntheticFolderRel: dataset.syntheticFolderRel,
    })), containerWorkspaceRoot);
    const packages = await (0, devcontainer_1.captureEnvironmentPackages)(language);
    (0, devcontainer_1.writeContainerFiles)({
        workspaceRoot,
        language,
        mounts,
    ...
    if (offerReopen) {
        const choice = await vscode.window.showInformationMessage(mounts.length > 0
            ...
            : "Ghosteado: Container protection refreshed. No synthetic mounts are prepared yet, so the container can generate code from schema but not run end-to-end reads.", "Reopen in Container", "Later");
...
async function resumeProtectedWorkspace(workspaceRoot) {
    const manifest = loadManifest(workspaceRoot);
    if (manifest.protectedDatasets.length === 0) {
        void vscode.window.showWarningMessage("Ghosteado: No protected datasets were found in this workspace.");
        return;
    }
    if ((0, devcontainer_1.isInsideContainer)()) {
        void vscode.window.showInformationMessage("Ghosteado: This workspace is already open inside the container.");
        return;
    }
    const issues = await validateProtectedWorkspacePaths(workspaceRoot, manifest);
    if (issues.length > 0) {
        void vscode.window.showErrorMessage(`Ghosteado: This protected workspace needs attention before reopening in the container.\n\n${formatIssueList(issues)}`);
        return;
    }
...
async function maybePromptToResume(workspaceRoot) {
    const manifest = loadManifest(workspaceRoot);
    if (manifest.protectedDatasets.length === 0 || (0, devcontainer_1.isInsideContainer)())
        return;
    if (resumePromptShown)
        return;
    resumePromptShown = true;
    const choice = await vscode.window.showInformationMessage("👻 Ghosteado detected a protected workspace. Resume in the container to keep AI work isolated from the real dataset.", "Resume in Container", "View Schema", "Later");
...
function showStatusPanel(workspaceRoot) {
    const manifest = loadManifest(workspaceRoot);
    const events = (0, logger_1.recentEvents)(workspaceRoot, 20);
    const items = [];
    items.push({
    ...
    void vscode.window
        ...
        if (selected.action === "openDatasetSchema" && selected.dataset) {
            ...
            const warningDoc = await vscode.workspace.openTextDocument((0, provider_1.warningUri)(selected.warningPath));
...
function updateStatusBar(workspaceRoot) {
    ...
    const inside = (0, devcontainer_1.isInsideContainer)();
    const prefix = inside ? "$(ghost) Container" : "$(ghost) Host";
    const blockSuffix = blockCount > 0 ? `  $(warning) ${blockCount} host warnings` : "";
...
function loadProtectedFolders() {
    const cfg = vscode.workspace.getConfiguration("ghosteado");
    const folders = (cfg.get("protectedFolders") ?? []).filter((folder) => fs.existsSync(folder));
    guard.setGhostedFolders(folders);
}
async function addProtectedFolder(folderPath) {
    const cfg = vscode.workspace.getConfiguration("ghosteado");
    const current = cfg.get("protectedFolders") ?? [];
    if (!current.includes(folderPath)) {
        await cfg.update("protectedFolders", [...current, folderPath], vscode.ConfigurationTarget.Workspace);
    }
}
async function removeProtectedFolder(folderPath) {
    const cfg = vscode.workspace.getConfiguration("ghosteado");
    const current = cfg.get("protectedFolders") ?? [];
    await cfg.update("protectedFolders", current.filter((item) => item !== folderPath), vscode.ConfigurationTarget.Workspace);
}
function ensureProjectDirectories(workspaceRoot) {
...
async function protectDatasetOnHost(workspaceFolderPath, realDatasetPath) {
...
function resolveLinkedDatasetPath(linkPath) {
    try {
...
function saveManifest(workspaceRoot, manifest) {
    ensureProjectDirectories(workspaceRoot);
    fs.writeFileSync(manifestPath(workspaceRoot), JSON.stringify(manifest, null, 2) + "\n", "utf8");
}
function saveSchema(workspaceRoot, dataset, schema) {
    ensureProjectDirectories(workspaceRoot);
    fs.writeFileSync(path.join(workspaceRoot, dataset.schemaFileRel), JSON.stringify(schema, null, 2) + "\n", "utf8");
}
function loadSchema(workspaceRoot, dataset) {
...
async function saveRealDatasetPath(workspaceRoot, datasetId, realDatasetPath) {
    const map = extensionContext.globalState.get(REAL_PATHS_KEY, {});
...
async function loadRealDatasetPath(workspaceRoot, datasetId) {
    const map = extensionContext.globalState.get(REAL_PATHS_KEY, {});
    return map[workspaceRoot]?.[datasetId];
}
async function removeRealDatasetPath(workspaceRoot, datasetId) {
    const map = extensionContext.globalState.get(REAL_PATHS_KEY, {});
    if (!map[workspaceRoot])
        return;
    delete map[workspaceRoot][datasetId];
    await extensionContext.globalState.update(REAL_PATHS_KEY, map);
}
async function pickProtectedDataset(workspaceRoot, manifest, uri, title) {
    if (uri) {
        const targetPath = path.resolve(uri.fsPath);
        const matched = manifest.protectedDatasets.find((dataset) => path.resolve(workspaceRoot, dataset.workspaceFolderRel) === targetPath);
        if (matched)
            return matched;
        void vscode.window.showWarningMessage(`Ghosteado: "${path.basename(targetPath)}" is not a protected dataset folder in this workspace.`);
        return undefined;
    }
    if (manifest.protectedDatasets.length === 0) {
        void vscode.window.showWarningMessage("Ghosteado: No protected datasets were found in this workspace.");
    ...
    const selected = await vscode.window.showQuickPick(manifest.protectedDatasets.map((dataset) => ({
        label: dataset.name,
        description: dataset.workspaceFolderRel,
...
async function promptForSyntheticRowCount() {
    const cfg = vscode.workspace.getConfiguration("ghosteado");
    const defaultValue = String(cfg.get("simulatedRowCount") ?? 100);
...
function ensureSyntheticWorkspace(workspaceRoot, dataset) {
    const dir = path.join(workspaceRoot, dataset.syntheticFolderRel);
    if (!fs.existsSync(dir))
        fs.mkdirSync(dir, { recursive: true });
}
function hasPreparedSyntheticWorkspace(workspaceRoot, dataset) {
    const dir = path.join(workspaceRoot, dataset.syntheticFolderRel);
    return fs.existsSync(dir) && fs.statSync(dir).isDirectory();
}
function promptFilePath(workspaceRoot, dataset) {
    return path.join(workspaceRoot, dataset.syntheticFolderRel, "SYNTHETIC_DATA_PROMPT.md");
}
function buildAnalysisPath(workspaceRoot, workspaceFolderRel) {
...
async function validateProtectedWorkspacePaths(workspaceRoot, manifest) {
    ...
    for (const dataset of manifest.protectedDatasets) {
        ...
        if (!linked && path.resolve(realDatasetPath) === path.resolve(workspaceFolderPath)) {
            issues.push(`"${dataset.name}" is no longer linked outside the workspace. Remove protection or protect the dataset again from the host workspace.`);
        }
    }
    return issues;
}
function containerProtectionNeedsRefresh(workspaceRoot, manifest) {
    const devcontainerPath = path.join(workspaceRoot, ".devcontainer", "devcontainer.json");
    if (!fs.existsSync(devcontainerPath))
        return true;
    try {
        const parsed = JSON.parse(fs.readFileSync(devcontainerPath, "utf8"));
        ...
        const containerWorkspaceRoot = (0, devcontainer_1.resolveContainerWorkspaceRoot)(workspaceRoot);
        const expectedMounts = (0, devcontainer_1.buildMountsFromProtectedDatasets)(workspaceRoot, manifest.protectedDatasets.map((dataset) => ({
            workspaceFolderRel: dataset.workspaceFolderRel,
            syntheticFolderRel: dataset.syntheticFolderRel,
        })), containerWorkspaceRoot).map(formatContainerMount);
        return (expectedMounts.length !== currentGhosteadoMounts.length ||
...
function formatContainerMount(mount) {
...
function mergePromptLanguages(languages) {
    const filtered = new Set(languages.filter((language) => language !== "none"));
    if (filtered.has("both"))
        return "both";
    if (filtered.has("r") && filtered.has("python"))
        return "both";
    if (filtered.has("r"))
        return "r";
    if (filtered.has("python"))
        return "python";
    return "none";
}
async function addSearchExclusion(folderPath, workspaceRoot) {
    const rel = toPosix(path.relative(workspaceRoot, folderPath));
    const wsConfig = vscode.workspace.getConfiguration();
    const searchExclude = {
        ...(wsConfig.get("search.exclude") ?? {}),
    };
    searchExclude[`${rel}/**`] = true;
    await wsConfig.update("search.exclude", searchExclude, vscode.ConfigurationTarget.Workspace);
}
async function removeSearchExclusion(folderPath, workspaceRoot) {
    const rel = toPosix(path.relative(workspaceRoot, folderPath));
    const wsConfig = vscode.workspace.getConfiguration();
    const searchExclude = {
        ...(wsConfig.get("search.exclude") ?? {}),
...
function writeIgnoreFiles(folderPath, workspaceRoot) {
    const rel = toPosix(path.relative(workspaceRoot, folderPath));
    const block = `\n${GHOST_TAG}\n${rel}/\n${rel}/**\n`;
    ...
    for (const fileName of [...IGNORE_FILENAMES, ".gitignore"]) {
        ...
        if (fs.existsSync(rootPath)) {
            ...
            if (!existing.includes(`${rel}/`))
                fs.appendFileSync(rootPath, block, "utf8");
        }
...
function removeIgnoreFiles(folderPath, workspaceRoot) {
    if (fs.existsSync(folderPath) && !fs.lstatSync(folderPath).isSymbolicLink()) {
        for (const fileName of IGNORE_FILENAMES) {
            const filePath = path.join(folderPath, fileName);
            if (fs.existsSync(filePath)) {
                const contents = fs.readFileSync(filePath, "utf8");
                if (contents.includes("Ghosteado"))
    ...
    for (const fileName of [...IGNORE_FILENAMES, ".gitignore"]) {
        ...
        if (after !== before)
            fs.writeFileSync(rootPath, after, "utf8");
    }
}
async function closeWarningTabs(placeholderUri) {
    for (const group of vscode.window.tabGroups.all) {
        for (const tab of group.tabs) {
            if (tab.input instanceof vscode.TabInputText &&
